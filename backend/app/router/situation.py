...
@router.get('/', response_model=List[schema.SituationOut], description='Get list of all situations')
...
def get_situation_by_id(db: Session = Depends(get_db), situationid: int = None):
    return service.get_situation_by_id(db, situationid)
...
def update_situation(db: Session = Depends(get_db), situationid: int = 0, situation: schema.SituationBase = Depends()):
    return service.update_situation(db, situationid, situation)


@router.patch('/', response_model=schema.SituationOut, description='Update situation fields')
def patch_situation(db: Session = Depends(get_db), situationid: int = 0, situation: schema.SituationBase = Depends()):
    return service.patch_situation(db, situationid, situation)
...
def delete_situation(db: Session = Depends(get_db), situationid: int = 0):
    return service.delete_situation(db, situationid)