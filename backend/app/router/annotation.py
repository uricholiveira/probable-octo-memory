...
from app.service import annotation as service, authentication as authentication_service, task as task_service
from app.schema import annotation as schema, authentication as authentication_schema

router = APIRouter(prefix='/annotation', tags=['Annotation'])


@router.get('/all', response_model=List[schema.AnnotationOut], description='Get list of all annotations')
...
def get_annotation_by_id(db: Session = Depends(get_db), annotation_id: int = None):
...
def update_annotation(db: Session = Depends(get_db), annotation_id: int = 0,
                      annotation: schema.AnnotationBase = Depends()):
    return service.update_annotation(db, annotation_id, annotation)


@router.patch('/', response_model=schema.AnnotationOut, description='Update annotation fields')
def patch_annotation(db: Session = Depends(get_db), annotation_id: int = 0,
                     annotation: schema.AnnotationPatch = Depends()):
    return service.patch_annotation(db, annotation_id, annotation)
