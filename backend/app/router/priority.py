...
@router.get('/', response_model=List[schema.PriorityOut], description='Get list of all priorities')
...
def get_priority_by_id(db: Session = Depends(get_db), priorityid: int = None):
    return service.get_priority_by_id(db, priorityid)
...
def update_priority(db: Session = Depends(get_db), priorityid: int = 0, priority: schema.PriorityBase = Depends()):
    return service.update_priority(db, priorityid, priority)


@router.patch('/', response_model=schema.PriorityOut, description='Update priority fields')
def patch_priority(db: Session = Depends(get_db), priorityid: int = 0, priority: schema.PriorityBase = Depends()):
    return service.patch_priority(db, priorityid, priority)
...
def delete_priority(db: Session = Depends(get_db), priorityid: int = 0):
    return service.delete_priority(db, priorityid)