...
def get_priority_by_id(db: Session, priorityid: int) -> Union[model.Priority, HTTPException]:
    priority = db.query(model.Priority).filter(model.Priority.id == priorityid).first()
    if not priority:
...
def update_priority(db: Session, priorityid: int, priority: schema.PriorityBase) -> Union[model.Priority, HTTPException]:
    updated_priority = get_priority_by_id(db, priorityid)
...
def patch_priority(db: Session, priorityid: int, priority: schema.PriorityBase) -> Union[model.Priority, HTTPException]:
    updated_priority = get_priority_by_id(db, priorityid)
...
def delete_priority(db: Session, priorityid: int) -> Union[JSONResponse, HTTPException]:
    priority = get_priority_by_id(db, priorityid)
    db.delete(priority)
    db.commit()
