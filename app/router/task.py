...
@router.post('/add-user', description='Add user to task', response_model=task_schema.TaskOut)
def add_user_to_task(db: Session = Depends(get_db), task: task_schema.TaskUserBase = Depends()):
    return task_service.add_user_to_task(db, task)


@router.post('/del-user', description='Delete user from task', dependencies=[Depends(authentication_service.oauth2_schema)])
def del_user_from_task(db: Session = Depends(get_db), taskid: int = 0, userid: int = 0):
    return task_service.del_user_from_task(db, taskid, userid)